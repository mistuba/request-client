...
func TestTruncationAndUI(t *testing.T) {
	...
	html := string(page)
	if strings.Contains(html, "未命名请求") || !strings.Contains(html, "发送") || !strings.Contains(html, "参数") || !strings.Contains(html, "压缩") || !strings.Contains(html, `id="rawHighlight"`) || !strings.Contains(html, `data-lang="en"`) {
